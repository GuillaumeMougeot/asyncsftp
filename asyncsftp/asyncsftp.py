...
class AsyncSFTP:
    ...
    async def _get(self, sftp: SFTPClient, remote_path: Path, local_path: Path):
        """
        Read remote file via asyncssh and write to local file (Windows-safe).
        """
        async with self.semaphore:
            try:
                ...
                if not data:
                    if self.do_log:
                        self.logger.warning(f"Skipping empty remote file: {remote_path}")
                    return
                ...
                if self.do_log:
                    self.logger.debug(f"Writing {len(data)} bytes to local file {local_path} in thread...")

                loop = asyncio.get_running_loop()
                # Write to local file using thread executor
                await loop.run_in_executor(None, local_path.write_bytes, data)

                if self.do_log:
                    self.logger.debug(f"Downloaded {remote_path} -> {local_path} ({len(data)} bytes).")
    ...
    async def _mget(self, sftp: SFTPClient, remotepaths: Union[list[str], str, Path], localdir: Union[str, Path]):
        """
        Recursively download multiple files or directories from remote to local.
        """
        try:
            if isinstance(remotepaths, (str, Path)):
                remotepaths = [str(remotepaths)]
            ...
            for rp in remotepaths:
                rp = str(rp)
                sftp_attrs = await sftp.stat(rp)

                if sftp_attrs.permissions & 0o040000:  # Directory (S_ISDIR)
                    # Recurse into directory
                    child_local_dir = localdir / posixpath.basename(rp)
                    child_local_dir.mkdir(parents=True, exist_ok=True)

                    # List remote directory contents
                    async for entry in sftp.scandir(rp):
                        remote_child = posixpath.join(rp, entry.filename)
                        tasks.append(self._mget(sftp, remote_child, child_local_dir))
                else:
                    local_file = localdir / posixpath.basename(rp)
                    tasks.append(self._get(sftp, rp, local_file))
    ...
    def mget(self, remotepaths: Union[str, list[str]], localdir: str, run: bool = False):
        """Get remote file(s) to local directory.
        """
        self.to_execute.append((self._mget, (remotepaths, Path(localdir)), {}))
        if run: self.run()
...
def main(share_link):
    ...
    if is_connected():
        ...
        connection.mput(
            localpaths=f"/media/pi/PiImages/{today}",
            remotedir=f"{today}",
        )
        connection.mput(
            localpaths=f"/media/pi/PiImages/CSV/{today}*",
            remotedir=f"results/detect",
        )
        connection.mput(
            localpaths=f"/media/pi/PiImages/tracks/{today}*",
            remotedir=f"results/tracks",
        )

        # Run the command in parallel
        connection.run()

        # Save the list of filenames from the remote folder
        # Run this after the folder upload has finished.
        connection.save_listdir(
            remote_folder=f"{today}",
            filenames_remote_folder="results/filenames",
            run=True,
        )
    else:
        connection.logger.error("Failed to connect to the internet.")
...
def test():
    connection = AsyncSFTP(sftp_params=AsyncSFTPParams(
            ...
            log_file="sftp.log")
    folder_path="D:\\"
    print(connection.list_missing_files(local_folder=folder_path, remote_folder="/"))
if __name__=='__main__':
    # parser = argparse.ArgumentParser()
    # parser.add_argument("-s", "--share_link", type=str, required=True, help="ERDA share link")
    # args = parser.parse_args()
    # main(args.share_link)
    test()